...
if MODE == "Generate":
	...
	for day in range(1, 21):
		print(f"Processing day: {day}")
		matches = pd.read_csv(f"cross_camera_matches/day_{day}.csv")
		ents_deps = pd.read_csv(f"entrances_and_departures/entrances_and_departures_day_{day}.csv")
...
demo_trajectories = {id: conf_sim_bbs.loc[conf_sim_bbs.id == id, \
							['camera', 'x1', 'y1', 'x2', 'y2']].to_numpy() \
							for id in conf_sim_objs.index.tolist()}
print(demo_trajectories)
